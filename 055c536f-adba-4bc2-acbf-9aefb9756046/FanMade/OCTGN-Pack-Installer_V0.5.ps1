...
ForEach($Pack in $nbPack) {
	...
	If(Extract-Packs -ArchivesToExtract $Pack) {
		...
		Try {
			...
			If($CardSetType -eq "VILLAIN") {
				$FolderToCheck = Join-Path -path $OCTGNDataFolder -childpath "$($MCModuleFolder)\FanMade\Villains"
				$FilesToMove = Get-ChildItem $FolderToCheck -File
				If($FilesToMove) {
					$FilesToMove | Where {$_.Name -like "*expert*"} | Move-Item -Destination "$($FilesToMove[0].Directory)\Expert" -Force
					$FilesToMove | Where {$_.Name -like "*standard*"} | Move-Item -Destination "$($FilesToMove[0].Directory)\Standard" -Force
				}
			}
		}
		Catch {
